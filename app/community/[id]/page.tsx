...
export default async function DiscussionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const month = months[discussion.created_at.getMonth()];
  const final = month + " " + discussion.created_at.getDate() + ", " + discussion.created_at.getFullYear();
  const final_like = discussion.like_count >= 1000
    ? (() => {
    ...
    if (today.toISOString().split('T')[0] == discussion.created_at.toISOString().split('T')[0]) {
        finalTime = `${today.getHours() - discussion.created_at.getHours()}h`;
    } else if (today.toISOString().slice(0, last_I) == discussion.created_at.toISOString().slice(0, last_I)) {
    ...
    } else if (today.toISOString().split("-")[0] == discussion.created_at.toISOString().split("-")[0]) {
        ...
        difference = today.getFullYear() - discussion.created_at.getFullYear() === 1;
  ...
  return (
    <div className="flex flex-col px-50 py-10  gap-gutter bg-surface-container-low">
      ...
      <div className="flex flex-row gap-sm items-center">
        <Link href={"/community"}><h1 className={`text-on-surface-variant text-headline-md uppercase`}>discussions</h1></Link>
        <ChevronRight/>
        <h1 className={`text-secondary text-headline-md uppercase`}>{discussion.subject_tag}</h1>
      </div>
      <div className="flex flex-row gap-margin">
        <div className="flex flex-col gap-margin  basis-2/3">  
            <div className="flex flex-col bg-surface-container-lowest p-margin rounded-xl border-1 border-outline-variant gap-lg">
                    ...
                    <h1 className={`font-serif text-display-lg font-bold`}>{discussion.title}</h1>
                    ...
                    <div className="border-b-1 border-outline-variant flex flex-row pb-md gap-20">
                        ...
                        <div className="flex flex-col gap-sm">
                            ...
                            <div className="flex flex-row items-center gap-sm">
                                ...
                                <h1  className="text-on-surface-variant text-label-md">{finalTime} ago</h1> 
                            </div>
                        </div>
                        <div className="flex flex-col gap-sm">
                    ...
                    <div className="flex flex-col gap-lg mt-md">
                        <h1 className="text-on-surface-variant text-body-lg">{discussion.content}</h1>
        ...
        <div className="basis-1/3 flex flex-col gap-margin">
                <div className="h-65 w-full bg-surface-container-lowest p-md  border-1 border-outline-variant rounded-xl flex flex-col gap-md">
                    ...
                    <div className="flex flex-row items-center gap-md">
                        <CircleUser size={50}/>
                        <div className="flex flex-col">
                        <h1 className="text-headline-lg font-serif">{discussion.author.name}</h1>
                        <h1 className="text-body-md text-primary">IB Educator</h1>
                        </div>                
                    </div>
                    <h1 className="text-body-md text-on-surface-variant">
                        {discussion.author.name} has contributed to over 20 high-scoring examplars to the IBPeople community. 
                    </h1>
